...
pub fn draw(ctx: jok.Context) !void {
    ...
    if (zgui.begin("Demo Settings", .{})) {
        ...
        zgui.sameLine(.{});
        zgui.textWrapped("Zig bindings for 'dear zgui' library. " ++
        ...
        if (zgui.collapsingHeader("Widgets: Input with Keyboard", .{})) {
            ...
            _ = zgui.inputText("Input text alloced", .{ .buf = alloced_input_text_buf });
            _ = zgui.text("length of Input text alloced {}", .{std.mem.len(alloced_input_text_buf.ptr)});
            _ = zgui.inputTextMultiline("Input text multiline alloced", .{ .buf = alloced_input_text_multiline_buf });
            _ = zgui.text("length of Input text multiline {}", .{std.mem.len(alloced_input_text_multiline_buf.ptr)});
            _ = zgui.inputTextWithHint("Input text with hint alloced", .{
                .hint = "Enter your name",
                .buf = alloced_input_text_with_hint_buf,
            });
            _ = zgui.text("length of Input text with hint alloced {}", .{std.mem.len(alloced_input_text_with_hint_buf.ptr)});
