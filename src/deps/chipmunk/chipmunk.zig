...
const PhysicsDebug = struct {
    ...
    fn addVertices(debug: *PhysicsDebug, vcount: u32, indices: []const u32) []sdl.Vertex {
        assert((@intCast(u32, debug.vattribs.items.len) + vcount) <= debug.max_vertex_num);
        const base_index = debug.vattribs.items.len;
        debug.vattribs.resize(debug.vattribs.items.len + @intCast(usize, vcount)) catch unreachable;
        debug.vindices.resize(debug.vindices.items.len + indices.len) catch unreachable;
        for (debug.vindices.items[debug.vindices.items.len - indices.len ..]) |*idx, i| {
            idx.* = indices[i] + @intCast(u32, base_index);
        }
        return debug.vattribs.items[debug.vattribs.items.len - @intCast(usize, vcount) ..];
    }
