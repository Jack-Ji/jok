...
pub fn add(self: *Self, name: []const u8, emitter: ParticleEmitter, opt: AddEffect) !*Effect {
    ...
    effect.* = .{
        ...
        .emitter = emitter,
        .origin = .new(opt.origin.x, opt.origin.y),
...
pub const FireEmitter = struct {
    ...
    age: f32 = 2,
    color_initial: jok.Color = .red,
    color_final: jok.Color = .yellow,
    color_fade_age: f32 = 1.5,
    ...
    fn emit(ptr: *anyopaque, random: std.Random, origin: Vector) Particle {
        ...
        const offset = Vector.new(
            random.float(f32) * self.radius * @cos(random.float(f32) * std.math.tau),
        ...
        return Particle{
            ...
            .move_acceleration = Vector.new(0, -random.float(f32) * self.acceleration),
            .move_damp = 0.96,
            .scale = 0.5,
            .scale_speed = -0.1,
